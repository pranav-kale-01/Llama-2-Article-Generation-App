...
def create_word_docx(user_input, paragraph, image_input):
    ...
    categories = dict(zip(encoder.fit_transform(df["category"]), df['category'].to_list()))

    # getting the tag for the articles using the sentmient model 
    with open( 'model.pkl', 'rb' ) as f: 
        clf = pickle.load(f)

    tags = clf.predict( [user_input] )
    article_tag = categories[tags[0]] 

    # Add the user input to the document
    doc.add_heading(user_input, level=1)

    # Adding the Tag to article
    doc.add_heading( "Tags: " + article_tag, level=1 )
